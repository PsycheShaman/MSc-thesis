...
nsigmaPion <- data.frame(rbind(cbind(real_nsig,"real"),cbind(sim_nsig,"sim")))

names(nsigmaPion) <- c("nsigmaPion","real_or_fake")

nsigmaPion$nsigmaPion <- as.numeric(as.character(nsigmaPion$nsigmaPion))
nsigmaPion$real_or_fake <- as.factor(nsigmaPion$real_or_fake)

ggplot(nsigmaPion,aes(nsigmaPion,fill=real_or_fake,colour=real_or_fake,alpha=0.2
                      ))+geom_freqpoly(bins=1000)
```

```{r}
selector <- sample(1:length(real_nsig),length(sim_nsig),replace=F)

real_nsig <- real_nsig[selector]

real_x <- real_x[selector,,]
