...
sd_el <- t(ph)[which(y==1),]
sd_el <- apply(sd_el, 2, sd)

sd_pi <- t(ph)[which(y==0),]
sd_pi <- apply(sd_pi, 2, sd)
...
ggplot(ph,aes(x=`Drift time (us)`,y=`Pulse height (mV)`,color=`Particle ID`))+geom_line()+xlim(c(0,2.8))+
  ...
  ylim(c(0,200))+
  theme(legend.box.background = element_rect(colour = "black"),
        legend.background = element_blank(),legend.key = element_blank())+theme(legend.position = c(0.9, 0.87),legend.text = 
                                                             element_text(size=12,  family="serif"),
                                                           legend.title = element_text(size=12,  family="serif"))+geom_point()
```


```{r}
ph2 <- data.frame(t(ph2))
...
plot(1:24*0.1,ph2_elec[1,],type="l",col="red",lty="solid",log=c("y"),ylim=c(1,3000),
     xlab=TeX("Drift time $(\\mu s)$"),
     ylab="Pulse height (mV)",main=TeX("$\\textit{e}$"))
for(i in 2:500){
  lines(1:24*0.1,ph2_elec[i,],col="red",lty="solid")
}
lines(1:24*0.1,ph_elec_mean,lty="solid",col="black",lwd=4)
# lines(1:24,ph_elec_mean+sd_el,lty="dashed",col="black",lwd=4)
# lines(1:24,ph_elec_mean-sd_el,lty="dashed",col="black",lwd=4)

ph2_pion <-
  ph2[y!=1,]
  
plot(1:24*0.1,ph2_pion[1,],ylim=c(1,3000),type="l",col="blue",lty="solid",log=c("y"),
     xlab=TeX("Drift time $(\\mu s$)"),
     ylab="Pulse height (mV)",main=TeX("$\\textit{\\pi}$"))
for(i in 2:500){
  lines(1:24*0.1,ph2_pion[i,],col="blue",lty="solid")
}
lines(1:24*0.1,ph_pi_mean,lty="solid",col="black",lwd=4)
# lines(1:24,ph_pi_mean+sd_pi,lty="dashed",col="black",lwd=4)
# lines(1:24,ph_pi_mean-sd_pi,lty="dashed",col="black",lwd=4)
