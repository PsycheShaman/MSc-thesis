---
title: "VQ-VAE (https://keras.rstudio.com/articles/examples/vq_vae.html)"
...
rm(proto)
proto <- keras_model_sequential() %>%
  default_conv(filters = base_depth, kernel_size = 3,input_shape=c(17,24,1))

summary(proto)

#17, 24, 32

proto <- proto %>% default_conv(filters = base_depth,
                   kernel_size = 3,
                   strides = 2)
...
proto <- proto %>% default_conv(filters = 2 * base_depth, kernel_size = 3)
...
proto <- proto %>%
  layer_flatten() %>%
  layer_dense(units = latent_size * code_size) %>%
...
encoder_model <- function(name = NULL,
  ...
  keras_model_custom(name = name, function(self) {
    self$conv1 <- default_conv(filters = base_depth, kernel_size = 3)
    self$conv2 <-
      default_conv(filters = base_depth,
                   kernel_size = 3,
                   strides = 2)
    self$conv3 <-
      default_conv(filters = 2 * base_depth, kernel_size = 3)
    self$conv4 <-
      default_conv(
        filters = 2 * base_depth,
        kernel_size = 3,
...
# input_size = latent_size * code_size
# 
# debug <- keras_model_sequential() %>%
#   layer_reshape(target_shape = c(1, 1, input_size),input_shape=c(16))
# 
# debug <- debug %>% #deconv1
#    default_deconv(
#         filters = 4 * base_depth,
#         kernel_size = 6,
#         padding = "valid"
#       )
# 
# 
# debug <- debug %>% #deconv2
#   default_deconv(filters = 2 * base_depth, kernel_size = 6)
# 
# debug <- debug %>% #deconv3
# default_deconv(
#         filters = 2 * base_depth,
...
# 
# debug <- debug %>% #deconv4
#   default_deconv(filters = base_depth, kernel_size = 6)
#   
# 
# debug <- debug %>% #deconv5
...
# 
# debug <- debug %>% #deconv6
# default_deconv(filters = base_depth, kernel_size = 3)
# 
# 
# debug <- debug %>%
#   default_conv(filters = 144,
#                    kernel_size = 1,
#                    activation = "linear")
# 
# summary(debug)
...
m <- keras_model_sequential() %>%
  layer_dense(144,input_shape = c(17,24,1)) %>%
  layer_dense(144) %>%
  layer_reshape(target_shape=c(17,24,1))
...
decoder_model <- function(name = NULL,
  ...
  keras_model_custom(name = name, function(self) {
    ...
    self$flatcake <- layer_flatten()
    
    self$conv1 <-
      layer_dense(units=26112,activation = "linear")
    
    self$resh <-
      layer_reshape(target_shape = c(64,17,24,1))
