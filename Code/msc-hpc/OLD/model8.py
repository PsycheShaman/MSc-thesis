...
model = Sequential()
model.add(Dense(1024,input_shape=(408,),activation="hard_sigmoid"))
...
plt.savefig('/home/vljchr004/msc-hpc/model5_history1.png', bbox_inches='tight')
...
plt.xlabel('epoch')
plt.legend(['train', 'test'], loc='upper left')
plt.savefig('/home/vljchr004/msc-hpc/model5_history2.png', bbox_inches='tight')

model.probs = model.predict_proba(x_test)

import numpy as np
np.savetxt("/home/vljchr004/msc-hpc/model5_results.csv", np.array(model.probs), fmt="%s")

np.savetxt("/home/vljchr004/msc-hpc/model5_y_test.csv", np.array(y_test), fmt="%s")

model.save('/home/vljchr004/msc-hpc/model5_.h5')  # creates a HDF5 file 'my_model.h5'
del model
