...
#        x = tf.layers.dense(x, units=inputs_decoder * 2 + 1, activation=lrelu)
        ...
        x = tf.layers.conv2d_transpose(x, filters=128, kernel_size=4, strides=2, padding='same', activation=tf.nn.relu)
        x = tf.nn.dropout(x, keep_prob)
        x = tf.layers.conv2d_transpose(x, filters=64, kernel_size=4, strides=1, padding='same', activation=tf.nn.relu)
        x = tf.nn.dropout(x, keep_prob)
        x = tf.layers.conv2d_transpose(x, filters=32, kernel_size=4, strides=1, padding='same', activation=tf.nn.relu)
        x = tf.layers.conv2d_transpose(x, filters=16, kernel_size=4, strides=1, padding='same', activation=tf.nn.relu)
        x = tf.nn.dropout(x, keep_prob)
        x = tf.contrib.layers.flatten(x)
...
#    plt.savefig('C:\\Users\\gerhard\\Documents\\deep-gen/vae4_res'+str(i)+'.png', bbox_inches='tight')
