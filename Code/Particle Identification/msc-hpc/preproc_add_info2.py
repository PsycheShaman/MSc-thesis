...
files = glob.glob("C:/Users/gerhard/Documents/msc-thesis-data/unprocessed/" + run + '/**/*.txt', recursive=True)
...
def file_reader1(i):
    ...
    if di == "}":
        ...
        di = literal_eval(di)
        ki = list(di.keys())
        pdgCode = [di.get(k).get('P') for k in ki]
        return(pdgCode)
...
with open('C:/Users/gerhard/Documents/msc-thesis-data/P_' + run + '.pkl', 'wb') as y_file:
