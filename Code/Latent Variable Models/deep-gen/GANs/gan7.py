...
def create_generator():
    ...
    generator.add(Dense(units=256,activation=tf.nn.leaky_relu))
    
    generator.add(Dense(units=512,activation=tf.nn.leaky_relu))
    
    generator.add(Dense(units=600,activation=tf.nn.leaky_relu))
...
def plot_generated_images(epoch, generator, examples=1, dim=(10,10), figsize=(17,24)):
    ...
    for i in range(generated_images.shape[0]):
        ...
        plt.axis('off')
        plt.tight_layout()
        plt.savefig('gan7_generated_image %d.png' %epoch)
