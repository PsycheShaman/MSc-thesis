...
sim_x <- read.csv("C:/Users/Gerhard/Documents/msc-thesis-data/simulated_datasets/simulated_data_aae12/sim_flat.csv",header=FALSE,sep=",",dec=".",nrows=1000000)

sim_x <- as.matrix(sim_x)

new_sim_x <- matrix(data=sim_x[1,],nrow=17,ncol=24,byrow = T)
dim(new_sim_x) <- c(1,17,24,1)

require(abind)

for(i in 2:1000000){
  a <- matrix(data=sim_x[i,],nrow=17,ncol=24,byrow=T)
  dim(a) <- c(1,17,24,1)
  new_sim_x <- abind(new_sim_x,a,along=1) 
}
...
new_sim_x = rescale(new_sim_x,to=c(min(real_x),max(real_x)))
```
