...
```{r}
save_model_hdf5(model, "C:/Users/gerhard/Documents/MSc-thesis/NEW/ML/model_focal_loss_mt_3_lt_4_GeV.h5", overwrite = TRUE,
  include_optimizer = TRUE)
```
...
```{r}
rm(pred_act,preds,six_tracklet_pred,six_tracklet_real,el,pi,x4,y4)
```
...
```{r}
load("C:/Users/gerhard/documents/msc-thesis-data/x_P5")
load("C:/Users/gerhard/documents/msc-thesis-data/y_P5")

# dim(x2)
dim(x5) <- c(dim(x5)[1],17,24,1)


preds <- predict(model,x5)

pred_act <- cbind(preds,y5)
...
```{r}
rm(pred_act,preds,six_tracklet_pred,six_tracklet_real,el,pi)
...
```{r}
save_model_hdf5(model, "C:/Users/gerhard/Documents/MSc-thesis/NEW/ML/model_focal_loss_mt_4_lt_5_GeV.h5", overwrite = TRUE,
  include_optimizer = TRUE)
```
...
model <- keras::load_model_weights_hdf5(model,filepath = "C:/Users/gerhard/Documents/MSc-thesis/NEW/ML/model_focal_loss_mt_4_lt_5_GeV.h5")
...
load("C:/Users/gerhard/documents/msc-thesis-data/y_P5")
load("C:/Users/gerhard/documents/msc-thesis-data/y_P6")
...
el2_train <- sample(el2,size=l2,replace = F)
pi2_train <- sample(pi2,size=l2,replace = F)

el3_train <- sample(el3,size=l3,replace = F)
pi3_train <- sample(pi3,size=l3,replace = F)

el4_train <- sample(el4,size=l4,replace = F)
pi4_train <- sample(pi4,size=l4,replace = F)

el5_train <- sample(el5,size=l5,replace = F)
pi5_train <- sample(pi5,size=l5,replace = F)

el6_train <- sample(el6,size=l6,replace = F)
pi6_train <- sample(pi6,size=l6,replace = F)


el2_test <- el2[-el2_train]
pi2_test <- pi2[-pi2_train]

el3_test <- el3[-el3_train]
pi3_test <- pi3[-pi3_train]

el4_test <- el4[-el4_train]
pi4_test <- pi4[-pi4_train]

el5_test <- el5[-el5_train]
pi5_test <- pi5[-pi5_train]

el6_test <- el6[-el6_train]
pi6_test <- pi6[-pi6_train]

y_train <- c(y2[c(el2_train,pi2_train)],
              y3[c(el3_train,pi3_train)],
                y4[c(el4_train,pi4_train)],
                   y5[c(el5_train,pi5_train)],
                      y6[c(el6_train,pi6_train)] )

y_test <- c(y2[c(el2_test,pi2_test)],
              y3[c(el3_test,pi3_test)],
                y4[c(el4_test,pi4_test)],
                   y5[c(el5_test,pi5_test)],
                      y6[c(el6_test,pi6_test)] )


save(y_train,file="C:/Users/gerhard/Documents/msc-thesis-data/final_y_train")
save(y_test,file="C:/Users/gerhard/Documents/msc-thesis-data/final_y_t")
...
load("C:/Users/gerhard/documents/msc-thesis-data/x_P5")
load("C:/Users/gerhard/documents/msc-thesis-data/x_P6")

x_train <- rbind(x2[c(el2_train,pi2_train),],
              x3[c(el3_train,pi3_train),],
                x4[c(el4_train,pi4_train),],
                   x5[c(el5_train,pi5_train),],
                      x6[c(el6_train,pi6_train),] )

x_test <- rbind(x2[c(el2_test,pi2_test),],
              x3[c(el3_test,pi3_test),],
                x4[c(el4_test,pi4_test),],
                   x5[c(el5_test,pi5_test),],
                      x6[c(el6_test,pi6_test),] )


save(x_train,file="C:/Users/gerhard/Documents/msc-thesis-data/final_x_train")
save(x_test,file="C:/Users/gerhard/Documents/msc-thesis-data/final_x_t")
```



```{r}
rm(x2,x3,x4,x5,x6,el2,el2_test,el2_train,el3,el3_test,el3_train,el4,el4_test,el4_train,el5,el5_test,el5_train,el6,el6_test,
   el6_train,
   pi2,pi2_test,pi2_train,pi3,pi3_test,pi3_train,pi4,pi4_test,pi4_train,pi5,pi5_test,pi5_train,pi6,pi6_test,
   pi6_train,
   y2,y3,y4,y5,y6)
...
# dim(x2)
dim(x_train) <- c(length(y_train),17,24,1)
...
model %>% compile(
  loss = fokLos(),
  optimizer = optimizer_adam(lr=0.0000001),
  metrics = c("accuracy")
)
...
save_model_hdf5(model, "C:/Users/gerhard/Documents/MSc-thesis/NEW/ML/model_GLOBAL_BALANCED.h5", overwrite = TRUE,
  include_optimizer = TRUE)
...
```{r}
load("C:/Users/gerhard/Documents/msc-thesis-data/final_x_train")
load("C:/Users/gerhard/Documents/msc-thesis-data/final_y_train")

load("C:/Users/gerhard/Documents/msc-thesis-data/final_y_t")
load("C:/Users/gerhard/Documents/msc-thesis-data/final_x_t")

test_el <- which(y_test==1)
test_pi <- which(y_test==0)

test_ind <- c(test_el,sample(test_pi,size=length(test_el),replace=F))
