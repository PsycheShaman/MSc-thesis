...
#    files_in_order = glob.glob("C:/Users/gerhard/Documents/msc-thesis-data/unprocessed/000" + run + '/**/*.txt', recursive=True)
    ...
    def file_reader1(i):
        ...
        if di == "}":
            ...
            di = literal_eval(di)
            ki = list(di.keys())
            Event = [di.get(k).get('Event') for k in ki]
            V0TrackID = [di.get(k).get('V0TrackID') for k in ki]
            track = [di.get(k).get('track') for k in ki]
            pdgCode = [di.get(k).get('pdgCode') for k in ki]
            nSigmaElectron = [di.get(k).get('nSigmaElectron') for k in ki]
            nSigmaPion = [di.get(k).get('nSigmaPion') for k in ki]
            PT = [di.get(k).get('PT') for k in ki]
            dEdX = [di.get(k).get('dEdX') for k in ki]
            P = [di.get(k).get('P') for k in ki]
            Eta = [di.get(k).get('Eta') for k in ki]
            Theta = [di.get(k).get('Theta') for k in ki]
            Phi = [di.get(k).get('Phi') for k in ki]
            
            det0 = [di.get(k).get('det0') for k in ki]
            row0 = [di.get(k).get('row0') for k in ki]
            col0 = [di.get(k).get('col0') for k in ki]
            layer0 = [di.get(k).get('layer 0') for k in ki]
            
            det1 = [di.get(k).get('det1') for k in ki]
            row1 = [di.get(k).get('row1') for k in ki]
            col1 = [di.get(k).get('col1') for k in ki]
            layer1 = [di.get(k).get('layer 1') for k in ki]
            
            det2 = [di.get(k).get('det2') for k in ki]
            row2 = [di.get(k).get('row2') for k in ki]
            col2 = [di.get(k).get('col2') for k in ki]
            layer2 = [di.get(k).get('layer 2') for k in ki]
            
            det3 = [di.get(k).get('det3') for k in ki]
            row3 = [di.get(k).get('row3') for k in ki]
            col3 = [di.get(k).get('col3') for k in ki]
            layer3 = [di.get(k).get('layer 3') for k in ki]
            
            det4 = [di.get(k).get('det4') for k in ki]
            row4 = [di.get(k).get('row4') for k in ki]
            col4 = [di.get(k).get('col4') for k in ki]
            layer4 = [di.get(k).get('layer 4') for k in ki]
            
            det5 = [di.get(k).get('det5') for k in ki]
            row5 = [di.get(k).get('row5') for k in ki]
            col5 = [di.get(k).get('col5') for k in ki]
            layer5 = [di.get(k).get('layer 5') for k in ki]
            return(pdgCode)
