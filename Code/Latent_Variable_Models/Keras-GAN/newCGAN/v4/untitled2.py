...
#            uniform(-1, 1, batch_size)
    ...
    def sample_images(self, epoch):
            ...
            plt.savefig("C:/Users/gerhard/documents/keras-gan/newCGAN/v4/images/pion_%d.png" % epoch)
            plt.close()
            plt.imshow(gen_imgs[1,:,:,0],cmap='gray')
            plt.title(label="Particle: e"+"; Charge sum: "+str(charge_sums[1])+"; After epoch: "+str(epoch))
            plt.savefig("C:/Users/gerhard/documents/keras-gan/newCGAN/v4/images/electron_%d.png" % epoch)
            plt.close()
...
#        labels = np.random.randint(0, self.num_classes,1000000).reshape(-1, 1)
        ...
        # Rescale images 0 - 1
        gen_imgs = 0.5 * gen_imgs + 0.5
        np.save("C:/Users/gerhard/documents/keras-gan/newCGAN/v4/simulated_data/"+str(epoch)+".npy",arr=gen_imgs)


if __name__ == '__main__':
    cgan = CGAN()
    cgan.train(epochs=50000000000000000000000000000000000001, batch_size=32, sample_interval=10)