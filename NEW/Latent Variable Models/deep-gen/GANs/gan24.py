...
def adam_optimizer():
    return tf.keras.optimizers.Adam(lr=0.000000001)
...
#    generator.add(tf.keras.layers.GaussianNoise(stddev=0.2))
    generator.add(Dense(units=300,activation='tanh'))
    generator.add(tf.keras.layers.Dropout(rate=0.2))
    generator.add(Dense(units=350,activation='tanh'))
    generator.add(tf.keras.layers.Dropout(rate=0.2))
    generator.add(Dense(units=400,activation='tanh'))
    generator.add(tf.keras.layers.Dropout(rate=0.2))
    generator.add(Dense(units=450,activation='tanh'))
    generator.add(tf.keras.layers.Dropout(rate=0.2))
    generator.add(Dense(units=500,activation='tanh'))
    generator.add(tf.keras.layers.Dropout(rate=0.2))
    generator.add(Dense(units=550,activation='tanh'))
    generator.add(tf.keras.layers.Dropout(rate=0.2))
    generator.add(Dense(units=600,activation='sigmoid'))
    generator.add(tf.keras.layers.Dropout(rate=0.2))
...
def create_discriminator():
    ...
    discriminator.add(tf.keras.layers.Dropout(rate=0.2))
...
def plot_generated_images(epoch, generator, examples=1, dim=(10,10), figsize=(17,24)):
    ...
    for i in range(generated_images.shape[0]):
        ...
        plt.axis('off')
        plt.tight_layout()
        plt.savefig('gan24_generated_image %d.png' %epoch)
