...
def create_generator():
    ...
    generator.add(Dense(units=25,input_dim=12,activation="tanh"))
    generator.add(Dense(units=50,activation="tanh"))
    generator.add(Dense(units=100,activation="tanh"))
    generator.add(Dense(units=150,activation="tanh"))
...
def create_discriminator():
    ...
    discriminator.add(tf.keras.layers.Conv2D(filters=16,kernel_size=(17,24),strides=1, padding='valid',activation="relu"))
...
def plot_generated_images(epoch, generator, examples=1, dim=(10,10), figsize=(17,24)):
    noise= np.random.normal(loc=0, scale=1, size=[examples, 12])
    ...
    for i in range(generated_images.shape[0]):
        ...
        plt.axis('off')
        plt.tight_layout()
        plt.savefig('gan14_generated_image %d.png' %epoch)
...
def training(epochs=1, batch_size=512):
    ...
    for e in range(1,epochs+1 ):
        ...
        #generate  random noise as an input  to  initialize the  generator
            noise= np.random.normal(0,1, [batch_size, 12])
