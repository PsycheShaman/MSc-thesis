...
```{r}
rm(list=ls())
for(model in c(paste0("model",c(25)))){
  print(model)
...
elec_pi_eff_func <- function(model_1.preds,model_1.labels){
  ...
  # names(pred_act)[2] <- "Particle_ID"
  
  pred_act$Particle_ID <- ifelse(pred_act[,2]==1,"electron","pion")
  
  pred_act$Particle_ID <- as.factor(pred_act$Particle_ID)
  
  g <- ggplot(pred_act,aes(pred_act[,1],colour=pred_act$Particle_ID))+geom_histogram(bins = 1000)+facet_wrap(pred_act$Particle_ID)+ggtitle("Model 25: Output probabilities for electrons: blue and pions: red")+
    xlab("Output probabilities per class")
  print(g)
