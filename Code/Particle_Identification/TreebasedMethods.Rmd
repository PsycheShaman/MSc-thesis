...
```{r}
gb_preds <- h2o.predict(gb,test.hex)

gb_preds <- as.data.frame(gb_preds)

preds <- as.data.frame(gb_preds$p1)
...
localH2O = h2o.init(min_mem_size = "20g",nthreads = -1)


# sv <- h2o.psvm(x=1:41,y=42,training_frame = train.hex,model_id = "sv1")
```

```{r}
nn <- h2o.deeplearning(x=1:24,y=25,training_frame = train.hex,nfolds=10)
```

```{r}
nn_preds <- h2o.predict(nn,test.hex)

nn_preds <- as.data.frame(nn_preds)

preds <- as.data.frame(nn_preds$p1)
...
```{r}
ensemble <- data.frame(cbind(gb_preds$p1,rf_preds$p1))

ensemble$preds <- rowMeans(ensemble)

```

```{r}
preds <- ensemble$preds
