...
def sample_images(models,data,epoch, latent_dim=3,batch_size=500000):
...
#    digit = x_decoded[0].reshape(17,24,1)
#            x_decoded = decoder.predict(z_sample)
...
#    end_range = (n - 1) * digit_size[1] + start_range + 1
...
#cat_inner = dill.loads(dill.dumps(categorical_focal_loss_fixed(gamma=2., alpha=.25)))
#print(cat_inner) 

reconstruction_loss = categorical_focal_loss_fixed(inputs,
                                                  outputs)
#
...
#        r, c = 5, 5
        noise = np.random.normal(0, 1, (200, 3))
        gen_imgs = decoder.predict(noise)
