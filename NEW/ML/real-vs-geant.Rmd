...
```{r}
selector <- sample(1:length(real_P),length(sim_P),replace = F)

real_Eta <- real_Eta[selector]
real_nsig <- real_nsig[selector]
real_P <- real_P[selector]
...
y <- c(rep(1,length(real_Eta)),rep(0,length(sim_Eta)))
...
rm(real_Eta,real_nsig,real_P,selector,sim_Eta,sim_nsig,sim_P,real_x,sim_x)
...
model <- keras_model_sequential() %>%
  layer_conv_2d(filters=16,kernel_size=c(2,3)) %>%
  layer_conv_2d(filters=32,kernel_size=c(3,3)) %>%
  layer_max_pooling_2d() %>%
  layer_flatten() %>%
  layer_dense(256,"sigmoid") %>%
  layer_dense(128,"sigmoid") %>%
